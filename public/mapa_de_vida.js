let casa, camino, ima, cruz, expo, lago, lago1, sam, samiyo;
let x = 0, y = 0;
let stp = 5;
let sz = 1.2;
let dst = 0;
...
function draw() {
  ...
  if (getX() > crzx - 48 && getX() < crzx + 48 && getY() > crzy - 48 && getY() < crzy + 48) {
  ...
  if (getX() > csax - 58 && getX() < csax + 58 && getY() > csay - 58 && getY() < csay + 58) {
    let d = dist(getX(), getY(), csax, csay);
    push();
    translate(csax, csay);
    scale(constrain(map(d, 0, 48, 1, 0), 0, 1));
